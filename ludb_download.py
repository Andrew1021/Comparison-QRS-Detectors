...
files = os.listdir("./lobachevsky-university-electrocardiography-database-1.0.1/lobachevsky-university-electrocardiography-database-1.0.1")

def read_ludb_files(files):
    """Utility function - read from database and store data
    """
    # Get signal
    dfs_ecg = []
    dfs_rpeaks = []
    ...
    for participant in range(200):
        ...
        data, info = wfdb.rdsamp(files + filename)
        ...
        anno = wfdb.rdann(files + filename, 'atr_i')
