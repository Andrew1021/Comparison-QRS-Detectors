...
def read_mit_bit_files():
    ...
    for participant, file in enumerate(data_files):
        ...
        if "x_" + file.replace("mit-bih-arrhythmia-database-1.0.0/", "") in os.listdir("mit-bih-arrhythmia-database-1.0.0/x_mitdb/"):
            ...
            data, anno = get_data_anno("mit-bih-arrhythmia-database-1.0.0/x_mitdb/" + "x_" + file.replace("mit-bih-arrhythmia-database-1.0.0/", ""), participant)
            # Store with the rest
            dfs_ecg.append(data)
            dfs_rpeaks.append(anno)
    ...
    # Save
    df_ecg = pd.concat(dfs_ecg).to_csv("data/ECGs.csv", index=False)
    dfs_rpeaks = pd.concat(dfs_rpeaks).to_csv("data/Rpeaks.csv", index=False)

    #return df_ecg, dfs_rpeaks
